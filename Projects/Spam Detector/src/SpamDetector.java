...
public class SpamDetector
{
	...
	private static final Pattern FEATURE_LINE = Pattern
			.compile("(\\w+),\\d+,([\\d\\.]+),([\\d\\.]+)");
	...
	public SpamDetector(InputStream is)
	{
		...
		while (s.hasNextLine())
		{
			...
			if (m.find())
			{
				WordFeature wf = new WordFeature(m.group(1), Integer.parseInt(m
	...
	{
		String subjectAndContent = email.getSubject() + " "
				+ email.getContent();
		String[] split = subjectAndContent.toLowerCase()
				.replaceAll("[!\\?\\.,]", "").split("\\s");
		...
		for (String s : split)
		{
			...
			{
				Double probability = wf.getSpamCount()
						/ (wf.getSpamCount() + wf.getHamCount()) + 0.005;
